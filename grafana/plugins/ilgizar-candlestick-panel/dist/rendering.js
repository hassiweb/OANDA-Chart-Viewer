...
System.register(['lodash', 'jquery', 'moment', 'jquery.flot', 'app/core/core', './vendor/jquery_flot_candlestick', './vendor/jquery_flot_axislabels'], function (_export, _context) {
  "use strict";

  var _, $, moment, appEvents;
  ...
  function link(scope, elem, attrs, ctrl) {
    ...
    function render_panel() {
      ...
      var lineWidth = panel.widthMode === 'auto' ? _.floor(panelWidth / (1.5 * data[0].stats.count)) : panel.candlestickWidth;
      if (!lineWidth) {
        lineWidth = 9;
      }
      ...
      var pushData = function pushData(i) {
      ...
      var pushIndicators = function pushIndicators(begin, end) {
        if (data.length > 4) {
    ...
    function showToolpit(pos) {
      ...
      var seriesItem = function seriesItem(label, value, color, line) {
        var seriesHtml = '<div class="graph-tooltip-list-item">';
        if (line) {
          seriesHtml += '<div class="graph-tooltip-series-name">' + '<i class="fa fa-minus" style="color:' + color + ';"></i> ' + label + ':</div>';
        } else {
      ...
      var body = '<div class="graph-tooltip-time">' + absoluteTime + '</div>' + seriesItem('Open', formatValue(data[0].datapoints[i][0]), grayColor, false) + seriesItem('High', formatValue(data[3].datapoints[i][0]), panel.colorizeTooltip && panel.mode === 'color' ? panel.bullColor : grayColor, false) + seriesItem('Low', formatValue(data[2].datapoints[i][0]), panel.colorizeTooltip && panel.mode === 'color' ? panel.bearColor : grayColor, false) + seriesItem('Close', formatValue(data[1].datapoints[i][0]), grayColor, false);
    ...
    function newDrawCandlestick(ctx, serie, data, hover) {
      ...
      function drawRange(ctx, serie, data) {
        var x = serie.xaxis.p2c(data[0]) + serie.candlestick.barWidth / 2;
        var y1 = serie.yaxis.p2c(data[3]);
        var y2 = serie.yaxis.p2c(data[4]);
      ...
      function drawBody(ctx, serie, data) {
        ...
        var x = serie.xaxis.p2c(data[0]) + half;
        var y1 = serie.yaxis.p2c(data[1]);
        var y2 = serie.yaxis.p2c(data[2]);
  ...
  return {
    ...
    }, function (_jqueryFlot) {}, function (_appCoreCore) {
      appEvents = _appCoreCore.appEvents;
    }, function (_vendorJquery_flot_candlestick) {}, function (_vendorJquery_flot_axislabels) {}],
    execute: function () {}
  };
});
